...
class BaseReporter(LoggerMixin, ABC):
    def __init__(self, hyper_parameters: dict):
        ...
        self.best_result = {"epoch": 0, "value": float("-inf")}
    ...
    def _set_git_info(self):
        try:
            ...
            self.git_diff = (
                subprocess.check_output("git diff -- . :(exclude)Pipfile.lock".split())
                .strip()
                .decode()[:49000]
            )
...
class GoogleSheetsReporter(BaseReporter):
    def __init__(self, sheets_id: str, auth_file_path: str, hyper_parameters: dict):
        ...
        for i, item in enumerate(sorted(self.hyper_parameters.items(), key=lambda x: x[0])):
            param_num = min(int(i / step) + 1, 4)
            self.template_engine.update_variables(
                **{f"params_{param_num}_key": item[0], f"params_{param_num}_value": item[1]}
            )
    ...
    def report_loss(self, loss_value: float, epoch: int):
        ...
        self.template_engine.update_variables(
            loss_epoch=epoch, loss=loss_value, duration=self.duration
        )
        self.template_engine.sync()
    ...
    def report_epoch(
        ...
        if dev_las > self.best_result["value"]:
            self.best_result = {"epoch": epoch, "value": dev_las}
    ...
    def finish(self, error=False):
        self.template_engine.update_variables(
            status="Finished" if not error else "Error", duration=self.duration
        )
        self.template_engine.sync()
