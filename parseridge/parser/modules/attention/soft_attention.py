...
from parseridge.parser.modules.attention.similarity_functions import DotSimilarity, \
    GeneralSimilarity, ScaledDotSimilarity, ConcatSimilarity, LearnedSimilarity
...
class Attention(Module):
    ...
    def __init__(self,
        ...
        if query_output_dim:
            self.query_transform = nn.Linear(
                ...
                out_features=query_output_dim
            )

        if key_output_dim:
            self.key_transform = nn.Linear(
                in_features=key_dim,
                out_features=key_output_dim
            )

        if value_output_dim:
            self.value_transform = nn.Linear(
                in_features=value_dim,
                out_features=value_output_dim
            )
    ...
    def forward(self, queries: Tensor, keys: Tensor, sequence_lengths: Tensor,
        ...
        if keys.size(1) == 0:
            ...
            dummy_attention_energies = torch.zeros(
                (keys.size(0), 1, 1), device=self.device)
