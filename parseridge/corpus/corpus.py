...
class Corpus(Dataset, LoggerMixin):
    ...
    def __init__(self, sentences, vocabulary, device="cpu"):
        ...
        for i, sentence in enumerate(self.sentences):
            sentence.id = i
            self.sentence_tensors.append([self._prepare_sentence(sentence)])
    ...
    def _prepare_sentence(self, sentence):
        ...
        return self._pad_list(tokens, max(self.sentence_lengths))

    def get_iterator(self, batch_size=32, shuffle=True, drop_last=False):
        """
        ...
        """
        return CorpusIterator(self, batch_size, shuffle, drop_last)
...
class CorpusIterator(LoggerMixin):
    ...
    def __init__(self, corpus, batch_size=48, shuffle=False, drop_last=False):
        """
    ...
    def __next__(self):
        if self._iter >= len(self):
            ...
            batch_tensors = self.corpus.sentence_tensors[indices_sorted]
