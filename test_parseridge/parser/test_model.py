...
def generate_dummy_sentence(length, size, offset=0):
    return torch.stack(
        tuple(
            [torch.rand((size,)).fill_(float(f"{i + 1}.{offset}")) for i in range(length)]
        )
    )
...
def generate_sentence_batch(batch_size, length, size):
    ...
    for i in range(batch_size):
        sentence_batch.append(generate_dummy_sentence(length, size, offset=i))
...
class TestParseProjective:
    ...
    def test_tensor_for_indices(self):
        ...
        output = AttentionModel.lookup_tensors_for_indices(
            indices_batch=stacks, sequence_batch=sentence_batch, padding=padding, size=size
        )
        ...
        assert np.array_equal(stack_1_output, np.array(expected_output_1, dtype=np.float32))
        ...
        assert np.array_equal(stack_2_output, np.array(expected_output_2, dtype=np.float32))
        ...
        assert np.array_equal(stack_3_output, np.array(expected_output_3, dtype=np.float32))
    ...
    def test_stack_buffer_concatenation(self):
        ...
        assert [int(n) for n in output.size()] == [
            batch_size,
            (stack_size + buffer_size) * embedding_size,
        ]
        ...
        assert np.array_equal(batch_1_output, np.array(expected_output_1, dtype=np.float32))
        ...
        assert np.array_equal(batch_2_output, np.array(expected_output_2, dtype=np.float32))
        ...
        assert np.array_equal(batch_3_output, np.array(expected_output_3, dtype=np.float32))
