...
class TestParseProjective:
    ...
    def test_sentence_parse(self):
        ...
        # Compute costs
        costs, shift_case = configuration.get_transition_costs(actions)
        assert costs == {T.LEFT_ARC: 3, T.RIGHT_ARC: 1, T.SHIFT: 0, T.SWAP: 1}
        assert shift_case == 2
        ...
        # Compute costs
        costs, shift_case = configuration.get_transition_costs(actions)
        assert costs == {T.LEFT_ARC: 1, T.RIGHT_ARC: 0, T.SHIFT: 1, T.SWAP: 1}
        assert shift_case == 2
        ...
        # Compute costs
        costs, shift_case = configuration.get_transition_costs(actions)
        assert costs == {T.LEFT_ARC: 2, T.RIGHT_ARC: 1, T.SHIFT: 0, T.SWAP: 1}
        assert shift_case == 2
        ...
        actions = generate_actions(
            T.LEFT_ARC, self.relations.relations, []
